 ...
 */
char*
file_read(const char* filename, int* size)
{
...
print_log(GLuint object)
{
  ...
  SDL_LogMessage(
    SDL_LOG_CATEGORY_APPLICATION, SDL_LOG_PRIORITY_ERROR, "%s\n", log);
  free(log);
}

/**
 * Compile the shader from source, with error handling
 */
GLuint
...
GLuint
create_program(const char* vertexfile, const char* fragmentfile)
{
  ...
  if (!link_ok) {
    SDL_LogMessage(
      SDL_LOG_CATEGORY_APPLICATION, SDL_LOG_PRIORITY_INFO, "glLinkProgram:");
    print_log(program);
...
GLuint
create_gs_program(const char* vertexfile,
                  ...
                  GLint vertices)
{
  ...
  if (!link_ok) {
    SDL_LogMessage(
      SDL_LOG_CATEGORY_APPLICATION, SDL_LOG_PRIORITY_ERROR, "glLinkProgram:");
    print_log(program);
    glDeleteProgram(program);
    return 0;
  }
